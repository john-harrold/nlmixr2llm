...
Codex enforces a default 32 KiB cap on combined `AGENTS.md` content (`project_doc_max_bytes` in `~/.codex/config.toml`). The agent plus all four skills is ~37 KiB, above the cap. The `nlmixr2verse` agent is ~8 KiB and is always included whole when agents are requested; `tasks = ...` subsets the skills (~8 KiB each). For Codex, pick the tasks you actually need:
